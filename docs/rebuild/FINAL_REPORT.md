...
5. **Server-Bestellversand aktivieren (Resend, D26)** — der Code ist fertig und schaltet sich selbst frei, sobald in Vercel die Env-Variable `RESEND_API_KEY` gesetzt ist (Projekt → Settings → Environment Variables, danach Redeploy). Dafür bei resend.com die Domain `sugarmoonsweets.de` verifizieren (DNS-Einträge im STRATO-Panel anlegen — **bestehende MX-Einträge nicht anfassen**, sonst fällt der normale Mailempfang aus) und einen API-Key erzeugen. Optional `SMS_ORDER_TO` (Empfänger, Default `sebastian.scherf@sugarmoonsweets.de`) und `SMS_ORDER_FROM` (verifizierter Absender, Default `bestellung@sugarmoonsweets.de`). **Vorher rechtlich klären:** AVV/DPA mit Resend abschließen und prüfen, ob die Datenschutzerklärung um den Versanddienst ergänzt werden muss — der Rechtstext wird von uns nicht verändert. Bis dahin läuft der bisherige mailto-Flow unverändert.
